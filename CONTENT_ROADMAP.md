...
- **Fresh signal audit before the next page**
  ...
  - 2026-07-11 GSC export: last 28 days showed 0 clicks and 59 impressions, but the final week improved to 36 impressions and Google started testing multiple strategic URLs: homepage 20 impressions at avg position 2.15, `/comparisons/hunter-vs-snovio` 15 at 7.27, `/category/ai-sales-prospecting-tools` 11 at 2.64, `/comparisons/chatbase-vs-intercom` 10 at 32.3, `/comparisons` 9 at 2.44, `tool/chatbase` 8 at 4.0, `tool/make` 8 at 8.25, and `/comparisons/hunter-vs-apollo` 7 at 17.0. Visible queries were sparse, but included `intercom knowledge base`, `apollo vs hunter.io`, `hunter apollo`, and `snov.io vs hunter.io`.
  - 2026-07-11 site action: shipped a small SEO/CTR package rather than a new section. Updated homepage metadata toward `B2B AI Tools Directory & Comparisons`, changed comparison page metadata to use the full comparison title, clarified Hunter.io naming in Hunter/Apollo and Hunter/Snov titles, expanded the Chatbase vs Fin page around Intercom knowledge base intent, and added ItemList + FAQPage schema with visible FAQ to `/category/ai-customer-support-tools`.
  - 2026-07-13 site action: added a moderated reviews foundation to tool pages. Reviews are submitted by email, stored as typed approved entries in `src/data/reviews.ts`, rendered visibly on the matching tool page, and only then included as `review`/`aggregateRating` in SoftwareApplication JSON-LD. Empty tool pages show a review form and no fake rating schema.
